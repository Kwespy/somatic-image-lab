  ...
  function open() {
    ...
    panel.innerHTML = `<div class="tsil-export-box" role="dialog" aria-modal="true" aria-label="${t.title}" style="max-height:90vh;overflow:auto">
      ...
      <div class="tsil-export-grid">
        <label>${t.format}<select data-format style="display:block;width:100%;padding:8px"><option value="story">${t.story}</option><option value="story-question">${t.storyQuestion}</option><option value="post">${t.post}</option><option value="link">${t.link}</option></select></label>
    ...
    q('[data-generate]').onclick = async () => {
      ...
      try {
        ...
        const variant = format === 'story' ? Math.floor(Math.random() * 8) : 0;
        const response = await fetch('/api/render', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-TSIL-Token': token }, body: JSON.stringify({ slug: data.slug, format, lang: language, variant }) });
