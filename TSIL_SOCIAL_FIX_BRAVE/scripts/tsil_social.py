...
def screenshot(html_text, out, size):
    ...
    if browser:
        ...
        try:
                ...
                if out.exists():
                    ...
                    if size_now > 5000:
                        if size_now == last_size:
                            ...
                            elif time.time() - stable_since > .35:
                                return
                        else:
                            stable_since = None
                            last_size = size_now
            ...
            if out.exists() and out.stat().st_size > 5000:
                return
            last_error = last_error or f'{browser_name or "El navegador"} no terminó el PNG dentro de 12 segundos.'
        finally:
...
def selected_browser_label():
    path, name = browser_bin()
    if path:
        return f"{name} — {path}"
...
def generate_assets(reading_dir, d):
    ...
    def make(label, html_text, out, size):
        nonlocal step
        step += 1
        out = Path(out)
        if out.exists() and out.stat().st_size > 5000:
            print(f'   [{step:02d}/{total}] {label} — ya existe ✓', flush=True)
        else:
            print(f'   [{step:02d}/{total}] {label} ...', flush=True)
            screenshot(html_text, out, size)
    ...
    for lang in ('es', 'en'):
        for i, style in enumerate(ASCII_STYLES, 1):
            ...
            make(f'Story {lang.upper()} {i}/8 · {style}', story_html(d, lang, style, render=True), out, (1080, 1920))

    for lang in ('es', 'en'):
        out = reading_dir / f'share-post-{lang}.png'
        make(f'Instagram 4:5 {lang.upper()}', post_html(d, lang, 'dense', render=True), out, (1080, 1350))

    og = reading_dir / 'share-og.png'
    make('Preview link 1200×630', approved_og_html(d), og, (1200, 630))
    ...
    print('   [ZIP] Empaquetando assets ...', flush=True)
